...
function ow_activate()
{
    ...
    add_option('ow_autopost_revision');
...
function ow_optionsAdmin()
{
    ...
    <script>
        function verifyAutoRevisions() {
            ...
            if (document.getElementById("ow_autopost_revision").checked) {
                confirm("Enabling this option may cause you to be banned from bookmarking services for excessive submissions.\n\n\'Cancel\' to stop, \'OK\' to enable it.") ? document.getElementById("ow_autopost_revision").checked = true : document.getElementById("ow_autopost_revision").checked = false;
            }
        }
        function auth() {
    ...
    ?>
            ...
            <input type="hidden" name="page_options" value="ow_username,ow_password,ow_service_logins,ow_autopost,ow_autopost_revision,ow_script,ow_autopost_enable" />
        ...
        else
        {
            if (count($networks->networks) > 0)
            {
                ...
                if (($ow_post_type != 'auto-draft') && ($ow_post_type != 'draft'))
                {
                    ?>
                    <label for="ow_post">
                        <input type="checkbox" <?php echo get_option('ow_autopost_revision') == 'on' ? 'checked="checked"' : ''; ?> id="ow_post" name="ow_post" /> Post this revision to OnlyWire	
                    </label>
